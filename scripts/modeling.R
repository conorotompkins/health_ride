...
df_daily_station <- data_wide %>% 
  ...
  #filter(station_name_type == "from_station_name") %>% 
  #filter(!is.na(usertype)) %>% 
  group_by(date, year, yday, month, wday, from_station_name, to_station_name) %>% 
  summarize(number_of_rides = n()) %>% 
  left_join(df_holidays) %>% 
  replace_na(replace = list(holiday = "none"))
df_daily_station
...
#model2 <- lm(number_of_rides ~ year + month + wday, data = df_daily_from)
#model3 <- lm(number_of_rides ~ year + month * wday, data = df_daily_from)
#model4 <- lm(number_of_rides ~ year + month * wday + holiday, data = df_daily_from)
#model5 <- lm(number_of_rides ~ year + month * wday + holiday + from_station_name, data = df_daily_from)
